...
export default function PdfLibraryScreen() {
  ...
  const downloadPdf = async (pdfId) => {
    try {
      ...
      Alert.alert(
        "Download Started",
        "The PDF is being downloaded to your device."
      );
      ...
      if (downloadResult && downloadResult.uri) {
        Alert.alert(
          "Success",
          "PDF downloaded successfully and is now available offline."
        );
            ...
            onPress: async () => {
              try {
                ...
                if (data.success) {
                  ...
                  setPdfs((currentPdfs) =>
                    currentPdfs.filter((pdf) => pdf.id !== pdfId)
                  );
        ...
        <View className="flex-row justify-between">
          ...
          <View className="flex-row">
            <TouchableOpacity
              ...
              className={`${
                isDownloaded ? "bg-gray-400" : "bg-green-500"
              } px-4 py-2 rounded-lg flex-row items-center mr-2`}
              disabled={isDownloaded}
            >
              <Ionicons
                name={
                  isDownloaded ? "checkmark-circle-outline" : "download-outline"
                }
                size={18}
                color="white"
                style={{ marginRight: 8 }}
