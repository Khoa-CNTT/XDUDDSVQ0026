...
export const checkAuthStatus = async () => {
  try {
    ...
    if (data.status && data.user) {
      ...
      if (!userId) {
        console.error(
          "API /user không trả về user_id. Vui lòng kiểm tra ứng dụng phía server."
        );
        await AsyncStorage.multiRemove([
          "token",
          "authToken",
        ...
        return {
          isLoggedIn: false,
          error: "Không nhận được ID người dùng từ API",
        };
      }
  ...
  try {
    ...
    if (data.status === true) {
      ...
      return {
        success: true,
        message:
          data.message || "Yêu cầu đặt lại mật khẩu đã được gửi thành công",
      };
    } else {
      return {
        success: false,
