...
export const saveBookReadingProgress = async (bookId, currentPage, totalPages) => {
  try {
    ...
    if (!bookId || typeof bookId !== 'string' || !bookId.startsWith('BOOK')) {
      ...
      return { success: false, error: 'INVALID_BOOK_ID', message: 'ID sách không hợp lệ' };
    }
    ...
    if (data.status) {
      ...
      console.log(`📚 Đã lưu tiến độ đọc sách lên server thành công: ${percentage}%`);
...
export const getBookReadingProgress = async (bookId) => {
  try {
    ...
    if (progressJson) {
      ...
      console.log(`📚 Đã lấy tiến độ đọc sách từ local storage: ${progress.percentage}%`);
      return { success: true, data: progress };
    }
...
const updateRecentlyViewedBooks = async (bookId) => {
  try {
    ...
    console.log(`Đã cập nhật danh sách sách đã xem cho người dùng ${userId}: ${recentlyViewed.length} sách`);
    ...
    await AsyncStorage.setItem('reading_progress_updated', new Date().toISOString()); // Giữ lại cho tương thích ngược
  } catch (error) {
...
export const addToRecentlyViewedBooks = async (bookId) => {
  try {
    ...
    const recentlyViewedJson = await AsyncStorage.getItem(storageKey);
    let recentlyViewed = recentlyViewedJson ? JSON.parse(recentlyViewedJson) : [];
    ...
    console.log(`📚 Đã lưu ${recentlyViewed.length} sách đã xem gần đây cho user ${userId}`);
