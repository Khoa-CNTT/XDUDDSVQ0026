...
export default function HomeScreen() {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (userId) {
          ...
          recentlyViewedIds = recentlyViewedJson ? JSON.parse(recentlyViewedJson) : [];
          console.log(`📚 Found ${recentlyViewedIds.length} recently viewed docs for user ${userId}`);
        } else {
  ...
  const sections = [
    { id: 'recentlyReadBooks', type: 'recentlyReadBooks', title: 'Sách Đã Đọc Gần Đây' },
    { id: 'recentlyRead', type: 'recentlyRead', title: 'Tài Liệu Đã Đọc Gần Đây' },
          ...
          <View>
            ...
            {processedBooks && processedBooks.length > 0 ? (
              ...
              <View className="px-4 py-6 items-center">
                <Text className="text-gray-500">Bạn chưa đọc sách nào gần đây</Text>
              </View>
            )}
          </View>
        );
  ...
  const checkAuthState = async () => {
    try {
      ...
      const recentlyViewedKey = userId ? `recently_viewed_books_${userId}` : 'recently_viewed_books';
      const recentlyViewedJson = await AsyncStorage.getItem(recentlyViewedKey);
      console.log(`📚 Recently viewed books key (${recentlyViewedKey}):`, recentlyViewedJson);
